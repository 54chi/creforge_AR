...
namespace Kudan.AR
{
    public class TrackerWindows : TrackerBase
    {
        ...
        public override bool StartInputFromCamera(int deviceIndex, int targetWidth, int targetHeight)
        {
            ...
            int camWidth = resolution[0];
            int camHeight = resolution[1];

            _clonedTexture = new Texture2D(camWidth, camHeight, TextureFormat.RGBA32, false);
        ...
        public override void UpdateTracking()
        {
            ...
            if (_numFramesProcessed != _numFramesTracked)
            {
                ...
                _projectionMatrix = ConvertNativeFloatsToMatrix(projectionFloats, (float)_width / (float)_height, _camWidth, _camHeight);
            }
        ...
        public override void updateCam()
        {
            ...
            long v = (long)texturePtr;

            NativeInterface.setTextureID(v);

            GL.IssuePluginEvent(NativeInterface.GetRenderEventFunc(), 0);
        }

        // Utility functions for converting native data into Unity data
        public static Matrix4x4 ConvertNativeFloatsToMatrix(float[] r, float cameraAspect, int camWidth, int camHeight)
        {
